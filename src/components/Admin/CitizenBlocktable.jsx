...
const CitizenBlocktable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchUsersBySabhaAndBlockStatus(sabhaId, true); // Assuming isBlock is false
        setUsers(data);
      } catch (error) {
  ...
  const handleClick = (userId) => {
    navigate(`/AdminViewBlockedCitizenDetails/${userId}`); // Navigate to the detailed view with userId
  };
